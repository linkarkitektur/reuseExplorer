...
namespace linkml {
    namespace markov_clustering
    {
        ...
        TEST("Normalizing a 3x3 sparce matrix"){
            ...
            for (int k = 0; k < expected.outerSize(); ++k)
                for (Eigen::SparseMatrix<double>::InnerIterator it(expected, k); it; ++it)
                    CHECK(matrix.coeff(it.row(), it.col()) == it.value());

        }
            
        TEST("Sparce Matrix inflated with power 2"){
            Eigen::SparseMatrix<double> matrix(3, 3);
            matrix.setIdentity();
            matrix.insert(2,0) = 2; 
            double power = 2.0;
        ...
        TEST("Sparce Matrix inflated with power 0.5"){
        ...
        TEST("Sparce Matrix expanded with power 2"){
            ...
            CHECK(matrix.coeff(1, 1) == 4.0);
            CHECK(matrix.coeff(2, 2) == 9.0);
        }
        
        TEST("Expanding an empty sparce matrix"){
        ...
        TEST("Sparce Matrix expanded with power 2.0 and random values"){
            Eigen::SparseMatrix<double> matrix(3, 3);
            ...
            CHECK( std::clamp(matrix.coeff(0, 0) - 6.72145533 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(0, 1) - 3.60563684 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(0, 2) - 4.52809956 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(1, 0) - 19.67902938, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(1, 1) - 8.31756915 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(1, 2) - 9.07857691 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(2, 0) - 17.57637361, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(2, 1) - 9.60940237 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(2, 2) - 9.58813282 , -1e-5, 1e-5));

        }

        TEST("Sparce Matrix expanded with power 5 and random values"){
            Eigen::SparseMatrix<double> matrix(3, 3);
            matrix.insert(0, 0) = 1.35707694;
            matrix.insert(0, 1) = 1.0320616;
            matrix.insert(0, 2) = 0.55297442;
            ...
            CHECK( std::clamp(matrix.coeff(0, 0) - 998.82208405 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(0, 1) - 497.49529662 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(0, 2) - 547.46786862 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(1, 0) - 2342.45929475, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(1, 1) - 1163.50184029, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(1, 2) - 1279.30965158, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(2, 0) - 2421.91378773, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(2, 1) - 1206.05182561, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(2, 2) - 1324.17647407, -1e-5, 1e-5));

        }
        
        TEST("Identity sparce Matrix with self-loops and value of 5"){
            // Create a sparse matrix
            Eigen::SparseMatrix<double> matrix(3, 3);
            matrix.insert(0, 0) = 1.0;
            ...
            CHECK(matrix.coeff(0, 1) == 1); // Check that the original values are unchanged
            CHECK(matrix.coeff(0, 0) == 5);
            CHECK(matrix.coeff(1, 1) == 5);
            CHECK(matrix.coeff(2, 2) == 5);
        }

        TEST("Empty sparce matrix with self-loops and value of 5"){
            // Create an empty sparse matrix
            Eigen::SparseMatrix<double> matrix(0, 0);
        ...
        TEST("Self loop with missing identety diagonal (Sparse)"){
        ...
        TEST("Pruning a 3x3 sparce matrix"){
            // Create a sparse matrix
            Eigen::SparseMatrix<double> matrix(3, 3);
            matrix.insert(0, 0) = 1.35707694;
            matrix.insert(0, 1) = 1.0320616;
        ...
        TEST("Pruning an empty sparce matrix"){
            // Create an empty sparse matrix
            Eigen::SparseMatrix<double> matrix(0, 0);
        ...
        TEST("Pruning a sparce matrix with threshold zero"){
            // Create a sparse matrix
            Eigen::SparseMatrix<double> matrix(3, 3);
            matrix.insert(0, 0) = 1.35707694;
            matrix.insert(0, 1) = 1.0320616;
        ...
        TEST("Check if two equal matrices converge (Sparse)"){
        ...
        TEST("Single MC iteration (Sparse)"){
            Eigen::SparseMatrix<double> matrix(3, 3);
            matrix.insert(0, 0) = 1.35707694;
            matrix.insert(0, 1) = 1.0320616;
            matrix.insert(0, 2) = 0.55297442;
            ...
            CHECK( std::clamp(matrix.coeff(0, 0) - 0.06093839, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(0, 1) - 0.0744922 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(0, 2) - 0.1052245 , -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(1, 0) - 0.52236217, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(1, 1) - 0.39640553, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(1, 2) - 0.42298073, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(2, 0) - 0.41669944, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(2, 1) - 0.52910227, -1e-5, 1e-5));
            CHECK( std::clamp(matrix.coeff(2, 2) - 0.47179475, -1e-5, 1e-5));
        }

        TEST("Get clusters from a 3x3 matrix([[1,0,0],[0,2,0],[0,0,3]]) (Sparse)"){
            Eigen::SparseMatrix<double> matrix(3, 3);
            matrix.insert(0, 0) = 1.0;
            matrix.insert(1, 1) = 2.0;
            matrix.insert(2, 2) = 3.0;
        ...
        TEST("Get clusters from a 3x3 matrix([[0,0,0],[1,1,1],[0,0,0]]) (Sparse)"){
            Eigen::SparseMatrix<double> matrix(3, 3);
            matrix.insert(1, 0) = 1;
            matrix.insert(1, 1) = 1;
            matrix.insert(1, 2) = 1;
        ...
        TEST("Markov Clustering on a 3x3 sparce matrix"){
            Eigen::SparseMatrix<double> matrix(3, 3);
            matrix.insert(0, 0) = 1.35707694;
            matrix.insert(0, 1) = 1.0320616;
            matrix.insert(0, 2) = 0.55297442;
